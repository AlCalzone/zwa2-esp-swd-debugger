...
The register-level rationale is in [swd-token-recovery.md](swd-token-recovery.md).
...
- **`verify/verify_tokens.py`** — read the tokens over a J-Link and check the byte
  order against the plan.

## Verify the byte order first

The plan's word tables are self-consistent. Whether the silicon stores the keys in
that order was unproven. A token written in the wrong order clears the wrong bits.
Undoing that means erasing the whole lockbits page. The erase also blanks the DSK
and QR data. So read a board with intact keys first. Use the firmware's `read`
command, or a J-Link with no firmware:

```bash
python3 verify/verify_tokens.py
```

`VERIFIED` means the write tables are safe. `BLANK` means you read a bricked board.
`MISMATCH` means do not write.

## Recovery procedure

1. Verify the byte order on a board with intact keys.
2. Flash this firmware onto the target's S3 and add the two jumpers.
3. Run `read`. A bricked board reads BLANK.
4. Run `write confirm`. It writes both spans and reads back VERIFIED.
5. Confirm the fix with an OTW update of controller firmware 1.2.0. Success is an
